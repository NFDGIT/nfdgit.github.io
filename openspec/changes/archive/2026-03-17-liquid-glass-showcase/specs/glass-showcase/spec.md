...
系统 SHALL 展示 `.glass-input` 输入框组件（如果尚未在 base.css 中定义则需新增），包括文本输入框、搜索框和文本区域。
...
系统 SHALL 展示所有设计令牌的实际渲染效果：颜色色板（accent、text、text-muted、glass-bg）、阴影层级、圆角尺寸对比、模糊程度对比。
...
系统 SHALL 使用多色渐变背景（CSS radial-gradient 叠加），使液态玻璃的半透明和模糊效果充分可见。
