...
系统 SHALL 将原单文件 `assets/js/note-browser.js` 中的逻辑按职责拆分为多个脚本文件（例如侧栏、目录树、预览、搜索等），并在 `note/index.html` 中以明确顺序加载；拆分后的对外行为 SHALL 仍满足既有 `note-browser` 能力规范（含中文路径与 hash 等），不因拆分引入功能回归。
